...
def download_http(
    ...
    """
    util.logger.info(f"Downloading file from {url} to {target}")

    precheck_url(url, timeout=precheck_timeout)
    
    requests.packages.urllib3.disable_warnings(InsecureRequestWarning) 

    req = requests.get(url, stream=True, headers={"User-Agent": "SeisBench"}, verify=False)
...
def precheck_url(url, timeout):
    ...
    if error_description is not None:
        util.logger.warning(
            ...
            f"network. "
            f"Please check https://github.com/easyQuake/easyQuake#known-issues for details."
        )
...
def callback_if_uncached(
    ...
    if exist():
        ...
        util.logger.info("Moving partial files to target")
